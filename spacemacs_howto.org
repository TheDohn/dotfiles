...
I am on ripgrep right now. That (and ag) seem to accept a hidden flag

at the command line this looks like:
$ ag --hidden "search terms"
or
$ rg --hidden "search terms"

and projectile search seems to work the same way, namely type "SPC /" to search a project, then add the hidden flag to the search:
-hidden "search term"
...
- just M-x for "query-replace-regexp", then type y for each replacement
- this works well when it is not a variable overall that I am changing, but maybe a prefix on a bunch of variables

- note that "replace-regexp" will replace all occurences all at once without confirmation
...
1) either using ag (SPC s a <various>)
or helm project search (SPC /)

2) then type CTRL-z will give options for copying output to a buffer, which can be useful for creating a TODO list based on search results
...
** DAP mode
-NOTE
1) for Python the Conda env must be activated for DAP mode to work!
2) Also debugpy must be installed in that Conda env!
...
* writing prose, letters etc.

** turn on auto-fill-mode to wrap lines as I type

the only issue here is that if you turn it on after you write a long line,
you need to run org-fill-paragraph to wrap line
* slow saving files, especially org

** some times saving my large org mode Journals gets slow so I need to clear the history
